...
from utilities.device_tpu import get_device, use_cuda
...
def main():
    ...
    for j in range(n_primers):
        ...
        pmp = torch.FloatTensor(pickle.load(open(f"{filepath}Pmp-{filename}-1.pickle", "rb"))).to(
            get_device()) if args.pmp else None
